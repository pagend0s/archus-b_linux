...
- **Data loss warning**: the selected USB device is **repartitioned** and **formatted**. Double-check prompts.
- **FAT type on ESP**: scripts format the ESP as **FAT16** and enforce USB ≤ **32 GB**. On modern systems, **FAT32** is the de-facto choice for UEFI ESPs. Consider switching to `mkfs.fat -F 32` and removing the 32 GB cap.
- **Host package names**:  
  - `e2label` and `fatlabel` are binaries; on Debian/Ubuntu they come from `e2fsprogs` and `dosfstools`, not from packages named exactly like the binaries. Adjust `apt install` lines if necessary.  
- **Hardcoded defaults**: timezone/keyboard are auto-detected from the host; root password is hardcoded; GRUB/kernel flags are opinionated.
- **Keyring trust**: signature verification imports a specific maintainer key (`pierre@archlinux.org`) via WKD. You may want to pin/check the full key fingerprint or use Arch release keys bundle.
